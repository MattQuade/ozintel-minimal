...
import {
  getLogisticsKegsArchiveDir,
  getLogisticsKegsArchiveFilePath,
  getLogisticsKegsFilePath,
  getLogisticsOpsDataDir,
} from "@/lib/dataPaths";
...
async function ensureDirs() {
  await fs.mkdir(getLogisticsOpsDataDir(), { recursive: true });
  await fs.mkdir(getLogisticsKegsArchiveDir(), { recursive: true });
}

async function writeStore(store: KegStore) {
  await ensureDirs();
  await fs.writeFile(getLogisticsKegsFilePath(), JSON.stringify(store, null, 2), "utf8");
}

async function archiveMonth(store: KegStore) {
  if (!store.entries.length && !store.month) return;
  await ensureDirs();
  const archivePath = getLogisticsKegsArchiveFilePath(store.month);
  try {
...
async function readStoreRaw(): Promise<KegStore> {
  await ensureDirs();
  const file = getLogisticsKegsFilePath();
  try {
...
export async function listArchiveMonths(): Promise<string[]> {
  await ensureDirs();
  try {
    const files = await fs.readdir(getLogisticsKegsArchiveDir());
    return files
...
export async function readArchive(
  ...
  try {
    const raw = await fs.readFile(getLogisticsKegsArchiveFilePath(month), "utf8");
