...
export default function LogisticsOperationsPage() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch("/api/operations/logistics/kegs");
  ...
  async function removeEntry(entry: KegEntry) {
    ...
    try {
      const res = await fetch(
        `/api/operations/logistics/kegs?id=${encodeURIComponent(entry.id)}`,
        { method: "DELETE" }
      );
      const data = await res.json();
      if (!res.ok || !data.success) {
  ...
  async function openArchive(m: string) {
    ...
    try {
      const res = await fetch(`/api/operations/logistics/kegs/archives/${m}`);
      const data = await res.json();
      if (!res.ok || !data.success) {
  ...
  return (
    <OpsGate permission="logisticsOps" section="Logistics Operations">
    <main
      style={{
    ...
    >
      ...
      <h1 style={{ color: "#0d9488", marginBottom: 8 }}>Logistics Operations</h1>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {archives.map((m) => (
              <button
                ...
                style={{
                  background: archiveMonth === m ? "#0f766e" : "#334155",
                  color: "white",
                  border: "none",
                  borderRadius: 8,
                  padding: "8px 12px",
