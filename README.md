...
- Receipt reading uses DeepSeek-OCR (via Replicate) when `REPLICATE_API_TOKEN` is set. Tesseract stays as the fallback, and as the only engine until that token exists.
