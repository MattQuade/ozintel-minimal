...
- Receipt shop and total use Amazon Textract AnalyzeExpense in `ap-southeast-2` (Sydney) when `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` are set. Tesseract is the fallback if those keys are missing or Textract does not return a total. The IAM user only needs `textract:AnalyzeExpense`. Stored receipt photos are still compressed on the phone to ~1280px JPEG quality 0.1.
