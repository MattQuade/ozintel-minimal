...
- Receipt shop and total use Amazon Textract in `ap-southeast-2` (Sydney) when `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` are set. Tesseract is only used if those keys are missing. The IAM user needs `AmazonTextractFullAccess` (or `textract:AnalyzeDocument`). Stored receipt photos are still compressed on the phone to ~1280px JPEG quality 0.1.
